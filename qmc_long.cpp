...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < eqsweeps; i++) {
		...
		diagonal_updates(spins, nspins, bonds, couplings, nbonds, opstring, 
				effexporder, temp, transfield, longfield, rng);
		...
		if (op_type == 0) {
			...
			if (coin < 1/(double)3) {
				...
				if (bj*s1*s2 > 0) {
					...
					//paccept = (1/temp) * nbonds * abs(bj) /(double) (effexporder - exporder); // (old) spin 1/2
				...
				//paccept = (1/temp) * nspins * transfield /(double) (effexporder - exporder); // (old) spin 1/2
			...
			if (opstring[p][1] > -1) {
				...
				//paccept = (effexporder - exporder + 1) /(double) ((1/temp) * nspins * transfield); // spin 1/2
			...
			//paccept = (effexporder - exporder + 1) /(double) ((1/temp) * nspins * transfield); // (old, wrong) spin 1/2
